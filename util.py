...
class TraceBack:
    ...
    def push(self,parentState, action, childState):
        self.dict[childState] = (parentState, action)
        
    def trace(self, State):
        path = list([])
        while State != self.startState:
            State, action = self.dict[State]
            path.append(action) 
...
"""
def MDWithWalls( xy1, xy2, wall):
    count1 = 0
    count2 = 0
    g = numpy.matrix(wall)
    for i in range(xy1[1],xy2[1]):
        if wall[xy1[0]][i] == True:
            count1 = count1 + 1
        if wall[xy2[0]][i] == True:
            count2 = count2 +1
    for j in range(xy1[0],xy2[0]):
        if wall[j][xy2[1]] == True:
            count1 = count1 + 1
        if wall[j][xy1[1]] == True:
            count2 = count2 +1
    return int(abs( xy1[0] - xy2[0] ) + abs( xy1[1] - xy2[1] ) + min(count1,count2)*3)
